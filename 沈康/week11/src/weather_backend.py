...
def get_location(city: str) -> str:
    ...
    Returns:
        返回 {"latitude": "纬度", "longitude": "经度", "address": "详细地址"} json字符串
    """
    with httpx.Client(timeout=10.0) as client:
        ...
        if not results:
            raise ValueError(f"未找到城市 '{city}'，请尝试其他写法（如'宁德市'改'宁德'）")
        ...
        address = f"{country} {admin1} {city_name}".strip()
        return json.dumps({"latitude": lat, "longitude": lon, "address": address}, ensure_ascii=False)


def get_weather(latitude: str, longitude: str, address: str) -> str:
    """
    通过经纬度坐标查询指定坐标位置的当前天气及未来3天预报。
...
if __name__ == "__main__":
    ...
    info = json.loads(get_location(args.city))
    print(get_weather(info["latitude"], info["longitude"], info["address"]))
